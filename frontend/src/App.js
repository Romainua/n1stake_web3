  ...
  render() {
    ...
    const logItems = (this.state.logs.length) ? this.state.logs.map((log,i) =>
      <div key={logMeta[i].timestamp}>
        <p className="label">
          <strong><span>Trend {(logMeta[i].type === 'increment') ? 'changed' : 'changed' }&nbsp;</span></strong>
        </p>
        <pre className="log-entry" key={i}>Success {(logMeta[i].type === 'increment') ? 'uptrend' : 'downtrend' }</pre>
      </div>
    ) : null;
